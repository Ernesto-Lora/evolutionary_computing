% For problem 1
function GA_binary(popSize, max_gen, cross_p, muta_p)
    % Initial population
    ...
    for gen = 1:max_gen
        ...
        for j = 1:length(father_indexes)/2
            ...
            % bin representation
            bin1 = representation1.gray_array(x1);
            bin2 = representation1.gray_array(x1);
            ...
            offsprings{2*j-1} = representation1.x_array_real(off1);
            offsprings{2*j} = representation1.x_array_real(off2);

        end

        fitness_offspring = [];
        for i = 1:popSize
            fitness_offspring(i) = fun1(offsprings{i});
        end
